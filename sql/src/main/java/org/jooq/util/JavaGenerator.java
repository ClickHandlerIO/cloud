...
public class JavaGenerator extends AbstractGenerator {
    ...
    private static final JooqLogger log = JooqLogger.getLogger(org.jooq.util.JavaGenerator.class);
    ...
    public final void generate(Database db) {
        ...
        this.database = db;
        this.database.addFilter(new org.jooq.util.JavaGenerator.AvoidAmbiguousClassesFilter());
    ...
    protected void generateRelations(SchemaDefinition schema) {
        ...
        for (TableDefinition table : database.getTables(schema)) {
            try {
                ...
                if (identity != null) {
                    final String identityType = out.ref(getStrategy().getFullJavaClassName(identity.getColumn().getContainer(), GeneratorStrategy.Mode.RECORD));
        ...
        for (TableDefinition table : database.getTables(schema)) {
            try {
                ...
                for (UniqueKeyDefinition uniqueKey : uniqueKeys) {
                    final String keyType = out.ref(getStrategy().getFullJavaClassName(uniqueKey.getTable(), GeneratorStrategy.Mode.RECORD));
        ...
        for (TableDefinition table : database.getTables(schema)) {
            try {
                ...
                for (ForeignKeyDefinition foreignKey : foreignKeys) {
                    final String keyType = out.ref(getStrategy().getFullJavaClassName(foreignKey.getKeyTable(), GeneratorStrategy.Mode.RECORD));
                    final String referencedType = out.ref(getStrategy().getFullJavaClassName(foreignKey.getReferencedTable(), GeneratorStrategy.Mode.RECORD));
    ...
    protected void printIdentity(JavaWriter out, int identityCounter, IdentityDefinition identity) {
        ...
        if (scala)
            out.tab(2).println("val %s : %s[%s, %s] = %s.createIdentity(%s, %s)",
                ...
                Identity.class,
                out.ref(getStrategy().getFullJavaClassName(identity.getTable(), GeneratorStrategy.Mode.RECORD)),
    ...
    protected void printUniqueKey(JavaWriter out, int uniqueKeyCounter, UniqueKeyDefinition uniqueKey) {
        ...
        if (scala)
            out.tab(2).println("val %s : %s[%s] = %s.createUniqueKey(%s, \"%s\", [[%s]])",
                ...
                UniqueKey.class,
                out.ref(getStrategy().getFullJavaClassName(uniqueKey.getTable(), GeneratorStrategy.Mode.RECORD)),
                AbstractKeys.class,
    ...
    protected void printForeignKey(JavaWriter out, int foreignKeyCounter, ForeignKeyDefinition foreignKey) {
        ...
        if (scala)
            out.tab(2).println("val %s : %s[%s, %s] = %s.createForeignKey(%s, %s, \"%s\", [[%s]])",
                ...
                ForeignKey.class,
                out.ref(getStrategy().getFullJavaClassName(foreignKey.getKeyTable(), GeneratorStrategy.Mode.RECORD)),
                out.ref(getStrategy().getFullJavaClassName(foreignKey.getReferencedTable(), GeneratorStrategy.Mode.RECORD)),
                AbstractKeys.class,
    ...
    protected void generateRecord(TableDefinition table) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(table, GeneratorStrategy.Mode.RECORD));
    ...
    protected void generateUDTRecord(UDTDefinition udt) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(udt, GeneratorStrategy.Mode.RECORD));
        log.info("Generating record", out.file().getName());
    ...
    private void generateRecord(Definition tableOrUdt, JavaWriter out) {
        final UniqueKeyDefinition key = (tableOrUdt instanceof TableDefinition)
            ...
            : null;
        final String className = getStrategy().getJavaClassName(tableOrUdt, GeneratorStrategy.Mode.RECORD);
        final String tableIdentifier = out.ref(getStrategy().getFullJavaIdentifier(tableOrUdt), 2);
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(tableOrUdt, GeneratorStrategy.Mode.RECORD));
        final List<? extends TypedElementDefinition<?>> columns = getTypedElements(tableOrUdt);

        printPackage(out, tableOrUdt, GeneratorStrategy.Mode.RECORD);
        ...
        for (int i = 0; i < degree; i++) {
            ...
            final String setter = getStrategy().getJavaSetterName(column, GeneratorStrategy.Mode.RECORD);
            ...
            if (generateInterfaces() && !generateImmutablePojos() && (isUDT || isArray)) {
                final String columnType = out.ref(getJavaType(column.getType(), GeneratorStrategy.Mode.RECORD));
                ...
                if (scala) {
                    ...
                    if (isUDT) {
                        ...
                        final String componentType = out.ref(getJavaType(array.getElementType(), GeneratorStrategy.Mode.RECORD));
        ...
        if (degree > 0 && degree <= Constants.MAX_ROW_DEGREE) {
            ...
            for (int i = 1; i <= degree; i++) {
                ...
                final String colType = out.ref(getJavaType(column.getType()));
                final String colGetter = getStrategy().getJavaGetterName(column, GeneratorStrategy.Mode.RECORD);
            ...
            for (int i = 1; i <= degree; i++) {
                ...
                final String colType = out.ref(getJavaType(column.getType()));
                final String colSetter = getStrategy().getJavaSetterName(column, GeneratorStrategy.Mode.RECORD);
        ...
        if (degree > 0 && degree < 256) {
            ...
            for (int i = 0; i < degree; i++) {
                ...
                final String columnMember = getStrategy().getJavaMemberName(column, GeneratorStrategy.Mode.DEFAULT);
                final String type = out.ref(getJavaType(column.getType()));
    ...
    protected void generateUDTInterface(UDTDefinition udt) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(udt, GeneratorStrategy.Mode.INTERFACE));
        log.info("Generating interface", out.file().getName());
    ...
    protected void generateUDT(UDTDefinition udt, JavaWriter out) {
        ...
        final String recordType = out.ref(getStrategy().getFullJavaClassName(udt, GeneratorStrategy.Mode.RECORD));
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(udt, GeneratorStrategy.Mode.DEFAULT));
    ...
    protected void generateArray(SchemaDefinition schema, ArrayDefinition array) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(array, GeneratorStrategy.Mode.RECORD));
    ...
    protected void generateEnum(EnumDefinition e) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(e, GeneratorStrategy.Mode.ENUM));
    ...
    protected void generateDomain(DomainDefinition d) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(d, GeneratorStrategy.Mode.DOMAIN));
    ...
    protected void generateDomain(DomainDefinition d, JavaWriter out) {
        final String className = getStrategy().getJavaClassName(d, GeneratorStrategy.Mode.DOMAIN);
        final String superName = out.ref(getStrategy().getJavaClassExtends(d, GeneratorStrategy.Mode.DOMAIN));
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(d, GeneratorStrategy.Mode.DOMAIN));
    ...
    protected void generateRoutines(SchemaDefinition schema) {
        ...
        for (TableDefinition table : database.getTables(schema)) {
            if (table.isTableValuedFunction()) {
                printTableValuedFunction(out, table, getStrategy().getJavaMethodName(table, GeneratorStrategy.Mode.DEFAULT));
            }
        }
    ...
    static String alias(int index) {
        ...
        int times = index / ALPHABET.length;
        int mod = index % ALPHABET.length;
        String s = "";
        for (int i = 0; i < times; i++) {
            s += "a";
        }
        return s + String.valueOf(ALPHABET[mod]);
    }
    /**
    ...
    protected void generateDao(TableDefinition table) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(table, GeneratorStrategy.Mode.DAO));
    ...
    protected void generatePojo(TableDefinition table) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(table, GeneratorStrategy.Mode.POJO));
    ...
    protected void generateUDTPojo(UDTDefinition udt) {
        JavaWriter out = newJavaWriter(getStrategy().getFile(udt, GeneratorStrategy.Mode.POJO));
        log.info("Generating POJO", out.file().getName());
    ...
    protected void generateTable(TableDefinition table, JavaWriter out) {
        ...
        final String recordType = out.ref(getStrategy().getFullJavaClassName(table, GeneratorStrategy.Mode.RECORD));
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(table, GeneratorStrategy.Mode.DEFAULT));
        final String schemaId = out.ref(getStrategy().getFullJavaIdentifier(schema), 2);
        ...
        if (scala) {
            out.println("class %s(alias : String, aliased : %s[%s], parameters : %s[ %s[_] ]) extends %s[%s](alias, %s, aliased, parameters, \"%s\")[[before= with ][separator= with ][%s]] {",
                className, org.jooq.Table.class, recordType, out.ref("scala.Array"), Field.class, TableImpl.class, recordType, schemaId, escapeString(comment), interfaces);
        } else {
        ...
        if (scala) {
            out.tab(1).println("private def this(alias : %s, aliased : %s[%s]) = {", String.class, org.jooq.Table.class, recordType);
            if (table.isTableValuedFunction())
            ...
            out.tab(1).println("private %s(%s alias, %s<%s> aliased) {", className, String.class, org.jooq.Table.class, recordType);
            if (table.isTableValuedFunction())
            ...
            out.println();
            out.tab(1).println("private %s(%s alias, %s<%s> aliased, %s<?>[] parameters) {", className, String.class, org.jooq.Table.class, recordType, Field.class);
        ...
        // type-safe table alias
            ...
            if (table.isTableValuedFunction())
                ...
                out.tab(2).println("return new %s(alias, %s);", className, identifier);
    ...
    protected void generateCatalog(CatalogDefinition catalog, JavaWriter out) {
        ...
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(catalog, GeneratorStrategy.Mode.DEFAULT));
    ...
    protected void generateSchema(SchemaDefinition schema, JavaWriter out) {
        ...
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(schema, GeneratorStrategy.Mode.DEFAULT));
    ...
    protected void generateRoutine(RoutineDefinition routine, JavaWriter out) {
        ...
        final List<String> interfaces = out.ref(getStrategy().getJavaClassImplements(routine, GeneratorStrategy.Mode.DEFAULT));
        final String schemaId = out.ref(getStrategy().getFullJavaIdentifier(schema), 2);
        ...
        for (ParameterDefinition parameter : routine.getInParameters()) {
            ...
            final String setter = getStrategy().getJavaSetterName(parameter, GeneratorStrategy.Mode.DEFAULT);
        ...
        for (ParameterDefinition parameter : routine.getAllParameters()) {
            ...
            if (isOutParameter && !isReturnValue) {
                ...
                final String paramType = out.ref(getJavaType(parameter.getType()));
                final String paramGetter = getStrategy().getJavaGetterName(parameter, GeneratorStrategy.Mode.DEFAULT);
                final String paramId = getStrategy().getJavaIdentifier(parameter);
    ...
    protected void printConvenienceMethodFunctionAsField(JavaWriter out, RoutineDefinition function, boolean parametersAsField) {
        ...
        if (scala)
            out.tab(1).print("def %s(",
                getStrategy().getJavaMethodName(function, GeneratorStrategy.Mode.DEFAULT));
        else
        ...
        for (ParameterDefinition parameter : function.getInParameters()) {
            final String paramSetter = getStrategy().getJavaSetterName(parameter, GeneratorStrategy.Mode.DEFAULT);
    ...
    protected void printConvenienceMethodFunction(JavaWriter out, RoutineDefinition function, boolean instance) {
        ...
        final String methodName = getStrategy().getJavaMethodName(function, GeneratorStrategy.Mode.DEFAULT);
    ...
    protected void printConvenienceMethodProcedure(JavaWriter out, RoutineDefinition procedure, boolean instance) {
        ...
        out.print(getStrategy().getJavaMethodName(procedure, GeneratorStrategy.Mode.DEFAULT));
        out.print("(");
        ...
        if (outParams.size() > 0) {
            ...
            final String getter = parameter == procedure.getReturnValue()
                ? "getReturnValue"
                : getStrategy().getJavaGetterName(parameter, GeneratorStrategy.Mode.DEFAULT);
            ...
            if (instance) {
                ...
                if (generateInterfaces() && isUDT) {
                    final String columnTypeInterface = out.ref(getJavaType(parameter.getType(), GeneratorStrategy.Mode.INTERFACE));
    ...
    protected void printConvenienceMethodTableValuedFunction(JavaWriter out, TableDefinition function, String javaMethodName) {
        ...
        final String recordClassName = out.ref(getStrategy().getFullJavaClassName(function, GeneratorStrategy.Mode.RECORD));
    ...
    protected void printRecordTypeMethod(JavaWriter out, Definition definition) {
        final String className = out.ref(getStrategy().getFullJavaClassName(definition, GeneratorStrategy.Mode.RECORD));
    ...
    protected void printPackage(JavaWriter out, Definition definition) {
        printPackage(out, definition, GeneratorStrategy.Mode.DEFAULT);
    }
    ...
    protected String getJavaType(DataTypeDefinition type) {
        return getJavaType(type, GeneratorStrategy.Mode.RECORD);
    }
    ...
    protected String getType(Database db, SchemaDefinition schema, String t, int p, int s, String u, String javaType, String defaultType) {
        return getType(db, schema, t, p, s, u, javaType, defaultType, GeneratorStrategy.Mode.RECORD);
    }
