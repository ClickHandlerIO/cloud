...
public class ZonedDateDeserializer extends JsonDeserializer<ZonedDate> {
  ...
  @Override
  public ZonedDate deserialize(JsonParser jsonParser, DeserializationContext deserializationContext)
    ...
    if (dateText.length() != 16
      ...
      throw new IOException(
          "Dates must be properly formed to ISO standard yyyy-MM-dd'T'HH:mm:ss'Z'");
    }

    // If date includes a timezone it must be UTC
    if ((dateText.length() == 17 || dateText.length() == 20 || dateText.length() == 24) && !dateText
        .endsWith("Z")) {
