...
public class ZonedDate {
   ...
   */
  public ZonedDate(int year, int monthOfYear, int dayOfMonth, int hourOfDay, int minuteOfHour,
      String zone) {
  ...
  public ZonedDate withHour(int hour) {
    LocalDateTime atHour = this.local.toLocalDateTime().withHour(hour).withMinute(0).withSecond(0)
        .withNano(0);
