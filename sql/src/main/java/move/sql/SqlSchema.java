...
public class SqlSchema {
  ...
  public SqlSchema(Connection connection, String catalog, String schema, boolean indexes)
      throws SQLException {
    ...
    try (final ResultSet rs = metaData
        .getTables(catalog.isEmpty() ? null : catalog, schemaPattern, null, tableTypes)) {
      while (rs.next()) {
    ...
    if (indexes) {
      ...
      for (DbTable table : tables.values()) {
        try (ResultSet rs = metaData
            .getIndexInfo(table.catalog, table.schema, table.name, false, false)) {
//            try (ResultSet rs = metaData.getIndexInfo(null, null, table.name.toUpperCase(), false, false)) {
          while (rs.next()) {
            ...
            if (index == null) {
              index = new DbIndex(cat, sch, tableName, unique, qualifier, name, type, cardinality,
                  pages);
  ...
  public static SqlSchema create(Connection connection, String catalog, String schema,
      boolean indexes) throws SQLException {
