 ...
 * deadline expires, the buffer makes a call to SQS to execute the current batch. <br> Internally,
 * the batch objects maintain a list of futures corresponding to the requests added to them. When a
 * batch completes, it loads the results into the futures and marks the futures as complete.
 */
public class SendQueueBuffer {
  ...
  /**
   * Current batching task for deleteMessage. Using a size 1 array to allow "passing by reference".
   * Synchronized by {@code deleteMessageLock}.
   */
  private final DeleteMessageBatchTask[] openDeleteMessageBatchTask = new DeleteMessageBatchTask[1];
  /**
   * Current batching task for changeMessageVisibility. Using a size 1 array to allow "passing by
   * reference". Synchronized by {@code changeMessageVisibilityLock}.
   */
  ...
  SendQueueBuffer(AmazonSQS sqsClient, Executor executor, QueueBufferConfig paramConfig,
      String url) {
   ...
   * @return never null
   */
  public QueueBufferFuture<SendMessageRequest, SendMessageResult> sendMessage(
      SendMessageRequest request,
      QueueBufferCallback<SendMessageRequest, SendMessageResult> callback) {
    QueueBufferFuture<SendMessageRequest, SendMessageResult> result = submitOutboundRequest(
        sendMessageLock,
   ...
   * @return never null
   */
  public QueueBufferFuture<DeleteMessageRequest, DeleteMessageResult> deleteMessage(
  ...
  public QueueBufferFuture<ChangeMessageVisibilityRequest, ChangeMessageVisibilityResult> changeMessageVisibility(
    ...
    return submitOutboundRequest(changeMessageVisibilityLock, openChangeMessageVisibilityBatchTask,
        request,
  ...
  /**
   * Flushes all outstanding outbound requests ({@code SendMessage}, {@code DeleteMessage}, {@code
   * ChangeMessageVisibility}) in this buffer. <p> The call returns successfully when all
   * outstanding outbound requests submitted before the call are completed (i.e. processed by SQS).
   */
  public void flush() {
  ...
  <OBT extends OutboundBatchTask<R, Result>, R extends AmazonWebServiceRequest, Result> QueueBufferFuture<R, Result> submitOutboundRequest(
    ...
    try {
      synchronized (operationLock) {
        if (openOutboundBatchTask[0] == null
          ...
          if (log.isTraceEnabled()) {
            log.trace(
                "Queue " + qUrl + " created new batch for " + request.getClass().toString() + " "
      ...
      Thread.currentThread().interrupt();
      AmazonClientException toThrow = new AmazonClientException(
  ...
  private abstract class OutboundBatchTask<R extends AmazonWebServiceRequest, Result> implements
     ...
     * addition failed.
     */
    public synchronized QueueBufferFuture<R, Result> addRequest(R request,
        QueueBufferCallback<R, Result> callback) {
    ...
    /**
     * Checks whether the buffer is now full. Called by {@code addIfAllowed} with a lock on {@code
     * this} held.
     *
    ...
    /**
     * Processes the batch once closed. Is <em>NOT</em> called with a lock on {@code this}. However,
     * it's passed a local copy of both the {@code requests} and {@code futures} lists made while
     * holding the lock.
     */
  ...
  private class SendMessageBatchTask extends
    ...
    protected boolean isOkToAdd(SendMessageRequest request) {
      return (requests.size() < config.getMaxBatchSize())
          && ((request.getMessageBody().getBytes().length + batchSizeBytes) < config
    ...
    @Override
    protected boolean isFull() {
      return (requests.size() >= config.getMaxBatchSize()) || (batchSizeBytes >= config
          .getMaxBatchSizeBytes());
    }
    ...
    protected void process(List<SendMessageRequest> requests,
      ...
      for (int i = 0, n = requests.size(); i < n; i++) {
        final SendMessageRequest request = requests.get(i);
        ...
        if (request.getMessageGroupId() != null && !request.getMessageGroupId().isEmpty()) {
          entry.setMessageGroupId(request.getMessageGroupId());
        }

        if (request.getMessageDeduplicationId() != null
            && !request.getMessageDeduplicationId().isEmpty()) {
          entry.setMessageDeduplicationId(request.getMessageDeduplicationId());
        }
  ...
  private class DeleteMessageBatchTask extends
      OutboundBatchTask<DeleteMessageRequest, DeleteMessageResult> {
    ...
    protected void process(List<DeleteMessageRequest> requests,
      ...
      for (int i = 0, n = requests.size(); i < n; i++) {
        entries
            .add(new DeleteMessageBatchRequestEntry().withId(Integer.toString(i)).withReceiptHandle(
  ...
  private class ChangeMessageVisibilityBatchTask extends
      OutboundBatchTask<ChangeMessageVisibilityRequest, ChangeMessageVisibilityResult> {
