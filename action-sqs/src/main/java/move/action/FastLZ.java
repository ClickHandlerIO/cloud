...
*/
  ...
  public static int compress(byte[] in, byte[] out) {
    if (in.length < 65536) {
      return compress_lz(in, out, defs.FASTLZ1);
    } else {
      return compress_lz(in, out, defs.FASTLZ2);
    }
  }
  ...
  static int compress_lz(byte[] in, byte[] out, defs compression_level) {
    ...
    while (ip < ipLimit) {
      ...
      if (!labelMatch) {
        ...
        if (distance == 0 || (compression_level == defs.FASTLZ1 ? distance >= 8192
            : distance >= 73725) //distance >= MAX_DISTANCE_LZ1 : distance >= MAX_FARDISTANCE_LZ2)
            || in[ref++] != in[ip++]
            || in[ref++] != in[ip++]
            || in[ref++] != in[ip++]) {
          out[op++] = in[anchor++];
  ...
  static int decompress(byte[] in, byte[] out, defs compression_level) throws Exception {
    if (compression_level == defs.FASTLZ0) {
    ...
    do {
      ...
      if (ctrl >= 32) {
        ...
        if (len == 6) { // (len == 7-1)
          if (compression_level == defs.FASTLZ1) {
            len += in[ip++] & 0xff;
          } else {
            do {
              code = in[ip++] & 0xff;
