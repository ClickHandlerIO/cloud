...
public final class TutorialTest {
  ...
  public void benchmark() throws IOException {
    ...
    final File path = new File(System.getProperty("user.home") + "/playground/lmdb");
    ...
    ByteBuffer bufV = ByteBuffer.allocateDirect(8);
    ...
    for (int p = 0; p < 10; p++) {
      try (Txn<ByteBuffer> txn = env.txnRead()) {
        ...
        try (CursorIterator<ByteBuffer> iterator = db
            .iterate(txn, KeyRange.openBackward(bufV, buf))) {
          for (KeyVal<ByteBuffer> buffer : iterator.iterable()) {
            System.out.println(buffer.key().getLong() + " -> " + buffer.val().getLong());
          }
        }

//      try (CursorIterator<ByteBuffer> iterator = db.iterate(txn, KeyRange.open(buf, bufV))) {
//        for (KeyVal<ByteBuffer> buffer : iterator.iterable()) {
//          System.out.println(buffer.key().getLong() + " -> " + buffer.val().getLong());
//        }
//      }
...
//      if (cursor.first()) {
//        System.out.println(cursor.val().getLong());
//      }
//
//      cursor.seek(SeekOp.MDB_FIRST);
//      buf.putLong(1002L);
//      buf.flip();
//      cursor.get(buf, GetOp.MDB_SET_KEY);
//
//      buf.putLong(1050L);
//      buf.flip();
//      cursor.get(buf, GetOp.MDB_SET_RANGE);
//
//      while (cursor.next()) {
//        System.out.println(cursor.val().getLong());
//      }
...
//    try (Txn<ByteBuffer> txn = env.txnRead()) {
//      System.out.println(UTF_8.decode(db.get(txn, key)).toString());
//    }
//
...
//    }
  ...
  public void tutorial1() throws IOException {
    ...
    try (Txn<ByteBuffer> txn = env.txnRead()) {
      System.out.println(db.get(txn, key));
    }
