...
import move.remoting.codegen.ComplexType;
import move.remoting.codegen.EnumType;
import move.remoting.codegen.ListType;
import move.remoting.codegen.MapType;
import move.remoting.codegen.MaterializedType;
import move.remoting.codegen.Namespace;
import move.remoting.codegen.RemotingRegistry;
import move.remoting.codegen.SetType;
import move.remoting.codegen.StandardType;
  ...
  public void generate() {
    ...
    try {
      com.google.common.io.Files.write(
          ...
          new File(
              outputDir + File.separator + rootPackage.replace(".", File.separator) + File.separator
                  + gwtModuleFileName)
      );
    } catch (IOException e) {
  ...
  private void generate(TypeSpec.Builder parentType, Namespace namespace) {
    if (namespace.isClass()) {
      ...
      if (namespace instanceof ActionSpec) {
        ...
        final Worker worker =
            (Worker) actionSpec.provider().getActionClass().getAnnotation(Worker.class);
        ...
        type.addMethod(MethodSpec.methodBuilder("path")
            ...
            .addStatement("return $S", worker.value().isEmpty() ? actionSpec.canonicalName() : worker.value()).build());
        ...
        responseEventBuilder.superclass(
            ParameterizedTypeName.get(ClassName.get(ResponseEvent.class), inType, outType));
  ...
  private void generate(TypeSpec.Builder parent, MaterializedType materializedType) {
    ...
    if (materializedType instanceof ComplexType) {
      ...
      if (ast.isPush(complexType)) {
        ...
        final String address = push != null && !push.value().isEmpty() ? push.value()
            : complexType.javaType().getSimpleName();
...
//                    type.superclass(ClassName.bestGuess(iface.canonicalName()));
        ...
        if (complexType.superType() != null && !Object.class
            .equals(complexType.superType().javaType())) {
//                type.addSuperinterface(ClassName.bestGuess(complexType.superType().canonicalName()));
        ...
        if (!materializedType.isAbstract() && !materializedType.isInterface()) {
          TypeSpec.Builder eventBuilder = TypeSpec.classBuilder("Event")
  ...
  private void generateComplexType(TypeSpec.Builder type, ComplexType complexType) {
    ...
    complexType.fields().forEach(field -> {
      ...
      if (field.field().getAnnotation(JsonIgnore.class) != null) {
        return;
      }
      ...
      if (field.type().canonicalName().equals("move.api.v1.common.ZonedDate")) {
        type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
            ...
            .addStatement("return this.$L == null ? null : Moment.moment(this.$L)", field.name(),
                field.name())
            .build());
        ...
        type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
            ...
            .addParameter(
                ParameterSpec.builder(TypeName.get(Moment.class), "value", Modifier.FINAL).build())
            .addStatement(
                "this.$L = value == null ? null : Moment.moment(value).tz(\"UTC\").toISOString()",
                field.name())
      ...
      type.addMethod(
          MethodSpec.methodBuilder(getterName).addModifiers(Modifier.PUBLIC, Modifier.FINAL)
              .returns(typeName)
              .addStatement("return $L", field.name())
              .addAnnotation(AnnotationSpec.builder(JsOverlay.class).build()).build());

      type.addMethod(
          MethodSpec.methodBuilder(setterName).addModifiers(Modifier.PUBLIC, Modifier.FINAL)
        ...
        case LIST:
          ...
          if (!arrayType.componentType().javaType().isPrimitive()) {
            try {
              // List accessors
              type.addMethod(
                  MethodSpec.methodBuilder(field.name() + "AsList").addAnnotation(JsOverlay.class)
                      .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                      .returns(ParameterizedTypeName.get(ClassName.get(List.class), valueTypeName))
                      .addStatement(
                          "return this.$L != null ? $T.asList(this.$L) : new java.util.ArrayList<>()",
                          field.name(), Arrays.class, field.name())
                      .build());
              type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
                  .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                  .returns(ClassName.bestGuess(complexType.canonicalName()))
                  ...
                  .addStatement(
                      "this.$L = value != null ? value.toArray(new $T[value.size()]) : null",
...
//                    if (!complexType.canonicalName().equals("move.api.v1.common.ZonedDate")) {
          type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
              ...
              .returns(ClassName.bestGuess(complexType.canonicalName()))
              .addParameter(
                  ParameterSpec.builder(TypeName.get(Moment.class), "value", Modifier.FINAL)
                      .build())
              .addStatement(
                  "this.$L = value == null ? null : Moment.moment(value).tz(\"UTC\").toISOString()",
                  field.name())
          ...
          type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
              .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
              .returns(TypeName.get(Moment.class))
              .addStatement("return this.$L == null ? null : $T.moment(this.$L)", field.name(),
                  Moment.class, field.name())
              .build());
//                    }
          break;
  ...
  private void generateLocators(Namespace namespace) {
    ...
    namespace.children().forEach((key, value) -> {
      if (value instanceof ActionSpec) {
        ...
        final ClassName className = ClassName
            .bestGuess(value.canonicalName() + ACTION_LOCATOR_SUFFIX);
        final String fieldName = lowercaseFirst(value.name());
