        ...
        complexType.fields().forEach(field -> {
            ...
            if (field.type().canonicalName().equals("move.api.v1.common.ZonedDate")) {
                ...
                type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
                        ...
                        .addCode(CodeBlock.of("if (value == null) { this.$L = null; } else { ZonedDate zonedDate = $T.create(); zonedDate.date = Moment.moment(value).format(\"YYYY-MM-DDTHH:mm:ss.SSS\"); zonedDate.zone = value.tz(); this.$L = zonedDate; } ", field.name(), Jso.class, field.name()))
                ...
                case DATETIME:
                    if (!complexType.canonicalName().equals("move.api.v1.common.ZonedDate")) {
                        type.addMethod(MethodSpec.methodBuilder(field.name()).addAnnotation(JsOverlay.class)
                                ...
                                .returns(ClassName.bestGuess(complexType.canonicalName()))
                                .addParameter(ParameterSpec.builder(TypeName.get(Moment.class), "value", Modifier.FINAL).build())
                                .addStatement("this.$L = value == null ? null : Moment.moment(value).format(\"YYYY-MM-DDTHH:mm:ss.SSS\")", field.name())
                                .addStatement("return this")
                                .build());
