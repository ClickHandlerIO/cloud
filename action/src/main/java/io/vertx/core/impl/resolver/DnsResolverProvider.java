...
public class DnsResolverProvider implements ResolverProvider {
  ...
  private final List<ResolverRegistration> resolvers = Collections
  ...
  public DnsResolverProvider(VertxImpl vertx, AddressResolverOptions options) {
    ...
    DnsServerAddresses nameServerAddresses =
        options.isRotateServers() ? DnsServerAddresses.rotational(serverList)
            : DnsServerAddresses.sequential(serverList);

    Map<String, Inet4Address> entries;
    if (options.getHostsPath() != null) {
      ...
      try {
        ...
        HostsFileParser.parse(file).inet4Entries();
        entries = HostsFileParser.parse(file).inet4Entries();
      } catch (IOException e) {
      ...
      entries = HostsFileParser.parseSilently().inet4Entries();
    }
    ...
    this.resolverGroup = new AddressResolverGroup<InetSocketAddress>() {
      ...
      @Override
      protected io.netty.resolver.AddressResolver<InetSocketAddress> newResolver(
          EventExecutor executor) throws Exception {
...
//          }
  ...
  private static class ResolverRegistration {
    ...
    ResolverRegistration(io.netty.resolver.AddressResolver<InetSocketAddress> resolver,
        EventLoop executor) {
