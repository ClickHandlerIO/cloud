...
public final class DefaultChannelId implements ChannelId {
    ...
    static {
        ...
        // Try Java 9's ProcessHandle API
        try {
            Class<?> processHandle = Class.forName("java.lang.ProcessHandle");
            Method current = processHandle.getMethod("current");
            Object instance = current.invoke(processHandle);
            Object pid = processHandle.getMethod("pid").invoke(instance);

            processId = ((Long)pid).intValue();
        } catch (Throwable e) {
            // Ignore.
        }
