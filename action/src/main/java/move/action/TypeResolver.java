...
public final class TypeResolver {
   ...
   * information from the {@code subType}. If no arguments can be resolved then {@code
   ...
   * @throws IllegalArgumentException if more or less than one argument is resolved for the {@code
   * type}
   */
   ...
   * resolved are returned as {@code Unknown.class}. If no arguments can be resolved then {@code
   * null} is returned.
   *
   * @param genericType to resolve arguments for
   * @param subType to extract type variable information from
   * @return array of raw classes representing arguments for the {@code genericType} else {@code
   ...
   */
  private static void populateSuperTypeArgs(final Type[] types,
    ...
    if (RESOLVES_LAMBDAS) {
      ...
      for (Method m : functionalInterface.getMethods()) {
        if (!isDefaultMethod(m) && !Modifier.isStatic(m.getModifiers()) && !m.isBridge()) {
          ...
          if (objectMethod != null && Arrays
              .equals(m.getTypeParameters(), objectMethod.getTypeParameters())) {
            continue;
          }
  ...
  private static boolean isAutoBoxingMethod(Method method) {
    ...
    return method.getName().equals("valueOf") && parameters.length == 1 && parameters[0]
        .isPrimitive()
