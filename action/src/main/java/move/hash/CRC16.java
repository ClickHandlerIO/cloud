...
public class CRC16 {
  ...
  public static int calc(byte[] bytes, int start, int end) {
    int crc = 0x0000;
  ...
  public static int calc(ByteBuf buf, int length) {
    ...
    for (int i = 0; i < length; i++) {
      crc = (crc >>> 8) ^ TABLE[(crc ^ buf.readByte()) & 0xff];
    }
    return crc;
  }
...
//        }
    ...
    for (int x = 0; x < 10; x++) {
      final LongSkipListMap<String> map = new LongSkipListMap<>();
      String bytes = NUID.nextGlobal();
...
//
//    for (int i = 0; i < 100; i++) {
//      byte[] bytes = NUID.nextGlobal().getBytes(Charsets.US_ASCII);
//      System.out.println("Thread: " + (calc(bytes, 0, bytes.length) % threadCount));
//    }
//
//    System.out.println("Table Based:");
//    for (int x = 0; x < 10; x++) {
//      byte[] bytes = NUID.nextGlobal().getBytes(Charsets.US_ASCII);
//
//      long start = System.currentTimeMillis();
