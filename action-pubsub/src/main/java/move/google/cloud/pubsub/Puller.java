...
public class Puller implements Closeable {
        ...
        .whenComplete((messages, ex) -> {
          ...
          if (ex != null) {
            if (ex instanceof RequestFailedException
                && ((RequestFailedException) ex).statusCode() == 429) {
          ...
          for (final ReceivedMessage message : messages) {
            ...
            try {
              handlerFuture = handler
                  .handleMessage(this, subscription, message.message(), message.ackId());
            } catch (Exception e) {
            ...
            handlerFuture
                .whenComplete((ignore, throwable) -> outstandingMessages.decrementAndGet());
  ...
  public static class Builder {
     ...
     * concurrency level of this {@link Puller}.</p>
     */
    ...
    /**
     * Set the limit of outstanding messages pending handling. Pulling is throttled when this limit
     * is hit. Default is {@code 64000}.
     */
    ...
    /**
     * Set the max size for the queue of acks back to Google Cloud Pub/Sub. Default is {@code 10 *
     * batchSize}.
     */
