 ...
 */
public final class PerMessageDeflateClientExtensionHandshaker implements
  ...
  public PerMessageDeflateClientExtensionHandshaker() {
    this(6, ZlibCodecFactory.isSupportingWindowSizeAndMemLevel(), MAX_WINDOW_SIZE, false, false, 0);
  }
   ...
   * @param allowClientWindowSize allows WebSocket server to customize the client inflater window
   * size (default is false).
   * @param requestedServerWindowSize indicates the requested sever window size to use if server
   * inflater is customizable.
   * @param allowClientNoContext allows WebSocket server to activate client_no_context_takeover
   * (default is false).
   * @param requestedServerNoContext indicates if client needs to activate
   * server_no_context_takeover if server is compatible with (default is false).
   */
  public PerMessageDeflateClientExtensionHandshaker(int compressionLevel,
      ...
      boolean allowClientNoContext, boolean requestedServerNoContext, int maxSize) {
    if (requestedServerWindowSize > MAX_WINDOW_SIZE
        || requestedServerWindowSize < MIN_WINDOW_SIZE) {
  ...
  public WebSocketClientExtension handshakeExtension(WebSocketExtensionData extensionData) {
    ...
    if (succeed) {
      return new PermessageDeflateExtension(serverNoContext, serverWindowSize,
          clientNoContext, clientWindowSize, maxSize);
    } else {
      return null;
    }
  }
  ...
  private final class PermessageDeflateExtension implements WebSocketClientExtension {
    ...
    public PermessageDeflateExtension(boolean serverNoContext, int serverWindowSize,
        boolean clientNoContext, int clientWindowSize, int maxSize) {
    ...
    public WebSocketExtensionDecoder newExtensionDecoder() {
      return new PerMessageDeflateDecoder(clientNoContext, maxSize);
    }
  }
