 ...
 */
public final class PerMessageDeflateServerExtensionHandshaker implements
    WebSocketServerExtensionHandshaker {

  public static final int SIZE_LOWER_BOUND = 8192;

  public static final int MAX_SIZE = 1024 * 1024 * 2;
   ...
   * @param allowServerWindowSize allows WebSocket client to customize the server inflater window
   * size (default is false).
   * @param preferredClientWindowSize indicates the preferred client window size to use if client
   * inflater is customizable.
   * @param allowServerNoContext allows WebSocket client to activate server_no_context_takeover
   * (default is false).
   * @param preferredClientNoContext indicates if server prefers to activate
   * client_no_context_takeover if client is compatible with (default is false).
   */
  public PerMessageDeflateServerExtensionHandshaker(int compressionLevel,
      ...
      int maxSize) {
    if (preferredClientWindowSize > MAX_WINDOW_SIZE
        || preferredClientWindowSize < MIN_WINDOW_SIZE) {
  ...
  public WebSocketServerExtension handshakeExtension(WebSocketExtensionData extensionData) {
    ...
    if (deflateEnabled) {
      return new PermessageDeflateExtension(compressionLevel, serverNoContext,
          serverWindowSize, clientNoContext, clientWindowSize, maxSize);
    } else {
      return null;
    }
  }
  ...
  private static class PermessageDeflateExtension implements WebSocketServerExtension {
    ...
    public WebSocketExtensionDecoder newExtensionDecoder() {
      return new PerMessageDeflateDecoder(serverNoContext, maxSize);
    }
