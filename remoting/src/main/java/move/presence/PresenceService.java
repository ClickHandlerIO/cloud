...
//        });
        ...
        PresenceOccupant occupant = (PresenceOccupant) map.submitToKey("1", new JoinProcessor().request(addRequest)).get();
//        occupant = (PresenceOccupant) map.submitToKey("1", new JoinProcessor().request(addRequest)).get();
        occupant = (PresenceOccupant) map.submitToKey("1", new JoinProcessor().request(addRequest2)).get();
        occupant = (PresenceOccupant) map.submitToKey("1", new JoinProcessor().request(addRequest3)).get();

        map.submitToKey("1", new ChangeStateProcessor().sessionId(addRequest2.sessionId).state("TYPING")).get();
...
//    }
    ...
    protected static class GetProcessor
        implements DataSerializable, EntryProcessor<String, Presence>, EntryBackupProcessor<String, Presence> {
        ...
        private void join(List<LeaveRequest> requests) {
            ...
            for (LeaveRequest request : requests) {
                try {
                    ...
                    LOG.error("Failed to 'leave' for key [" + request.key + "'] and session ['" + request.sessionId + "']", e);
                    request.future = null;
