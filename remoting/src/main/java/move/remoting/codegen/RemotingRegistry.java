  ...
  public void construct() {
    ...
    types.forEach((type, spec) -> {
      if (spec instanceof MaterializedType) {
        ...
        final String canonicalName = findCanonicalName(
            TypeToken.of(type).getRawType().getCanonicalName());
        ...
        if (materializedType.namespace().isClass() && !Modifier
            .isStatic(materializedType.javaType().getModifiers())) {
    ...
    if (!nonStaticTypes.isEmpty()) {
      nonStaticTypes.forEach(t -> System.err
          .println("Type [" + t.javaType().getCanonicalName() + "] needs the 'static' modifier."));
  ...
  private StandardType buildType(Class type, Type genericType) {
    try {
      ...
      if (type.isEnum()) {
        ...
        final Class componentType = TypeToken.of(genericType)
            .resolveType(javaslang.collection.Set.class.getTypeParameters()[0]).getRawType();
        dataType = new ListType(buildType(componentType));
      } else if (javaslang.collection.Seq.class.isAssignableFrom(type)) {
        final Class componentType = TypeToken.of(genericType)
            .resolveType(javaslang.collection.Seq.class.getTypeParameters()[0]).getRawType();
        dataType = new ListType(buildType(componentType));
      } else if (List.class.isAssignableFrom(type)) {
        final Class componentType = TypeToken.of(genericType)
            .resolveType(List.class.getTypeParameters()[0]).getRawType();
        dataType = new ListType(buildType(componentType));
      } else if (Set.class.isAssignableFrom(type)) {
        final Class componentType = TypeToken.of(genericType)
            .resolveType(Set.class.getTypeParameters()[0]).getRawType();
        ...
        final Class keyType = TypeToken.of(genericType)
            .resolveType(Map.class.getTypeParameters()[0]).getRawType();
        final Class valueType = TypeToken.of(genericType)
            .resolveType(Map.class.getTypeParameters()[1]).getRawType();
        ...
        if (dataType == null) {
          if (Action.class.isAssignableFrom(type)) {
            ...
            for (Field field : declaredFields) {
              ...
              final Method fluentGetter = Try.of(() -> type.getDeclaredMethod(name))
                  .getOrElse(none);

              Method getter = Try.of(() -> type.getDeclaredMethod("get" + capitalize(name)))
                  .getOrElse(none);
              if (getter == null) {
                getter = Try.of(() -> type.getDeclaredMethod("is" + capitalize(name)))
                    .getOrElse(none);
              }
              ...
              String jsonName =
                  jsonProperty != null ? Strings.nullToEmpty(jsonProperty.value()).trim() : "";
